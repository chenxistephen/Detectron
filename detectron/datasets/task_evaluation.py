...
def evaluate_all(
    dataset, all_boxes, all_segms, all_keyps, output_dir, use_matlab=False,computeLocPR=False
):
    ...
    all_results = evaluate_boxes(
        dataset, all_boxes, output_dir, use_matlab=use_matlab,computeLocPR=computeLocPR
    )
...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False, computeLocPR=False):
    ...
    if _use_json_dataset_evaluator(dataset):
        coco_eval = json_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=not_comp,computeLocPR=computeLocPR
        )
