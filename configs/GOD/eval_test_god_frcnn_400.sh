...
CUDA_VISIBLE_DEVICES="$GPUS" python tools/test_net.py \
    ...
    TEST.DATASETS "('GOD_Object365_val', 'GOD_FashionV2_val','GOD_furniture_val','GOD_coco_2014_minival','GOD_OpenImage_val')" \
    TEST.SOFT_NMS.ENABLED $SOFTNMS \
    NUM_GPUS $NUM_GPUS \
    TEST.SCALE $SCALE \
    USE_NCCL True \
    | tee $OUTPUT_DIR/eval_$MODEL_NAME-Test_$SCALE-SoftNMS-$SOFTNMS.log
    #--range 1000 2000 \
    #--compute_loc_pr\
